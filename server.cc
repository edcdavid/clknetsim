...
/* Global flag for graceful shutdown of infinite simulations */
static volatile int simulation_running = 1;

/* Global network pointer for signal handler access */
static Network *global_network = NULL;

/* Signal handler for graceful shutdown */
static void signal_handler(int sig) {
	simulation_running = 0;
	if (global_network) {
		global_network->stop_infinite_simulation();
	}
...
int main(int argc, char **argv) {
	...
	int infinite_mode = 0;  /* Flag for infinite simulation */
	...
	while ((opt = getopt(argc, argv, "l:r:R:n:o:f:Gg:p:s:v:e:ih")) != -1) {
		switch (opt) {
			case 'l':
				limit = atof(optarg);
				break;
	...
	if (optind + 2 != argc || help) {
		...
		printf("       -i            run simulation infinitely (until signal)\n");
		printf("       -G            print num numbers generated by expr\n");
		printf("       -h            print usage\n");
		return 1;
	}

	/* Set up signal handlers for infinite mode */
	if (infinite_mode) {
		signal(SIGINT, signal_handler);
		signal(SIGTERM, signal_handler);
		fprintf(stderr, "Infinite mode enabled. Use Ctrl+C or send SIGTERM to stop.\n");
	}
	...
	/* Set global network pointer for signal handler */
	...
	if (infinite_mode) {
		fprintf(stderr, "Running infinite simulation...");
		/* Reset infinite simulation flag in case it was stopped before */
		network->reset_infinite_simulation();
	} else {
	...
	if (r) {
		if (infinite_mode) {
			/* Run simulation infinitely until signal */
			r = network->run_infinite();
		} else {
			r = network->run(limit);
		}
	}
	...
	/* Clean up global network pointer */
	global_network = NULL;
	delete network;
